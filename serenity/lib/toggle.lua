	...
	if true then
        ...
        function(input)
			...
			TextButton.MouseButton1Down:Connect(function()
				if not focus then
					TextLabel.Text = "ON"
					ToggledUi.Enabled = false
				else
					TextLabel.Text = "OFF"
					ToggledUi.Enabled = true
				end
				focus = not focus
			end)

			local EnumKeyCode = Enum.KeyCode.LeftControl
			if getgenv().KeyCode ~= nil then
				EnumKeyCode = getgenv().KeyCode
			end
